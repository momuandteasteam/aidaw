...
export class ContentCatalog {
 ...
 async search(query:string,hostFamily:string|undefined,limit:number,offset:number){const c=await this.read(),tokens=query.toLocaleLowerCase().split(/\s+/).filter(Boolean),catalog=await this.service.catalog();
  ...
  const items=[];for(const i of matched.slice(offset,offset+limit)){const exists=await stat(i.path).then(s=>s.isFile()).catch(()=>false),hostCandidates=catalog.plugins.filter(p=>p.instrument&&i.family!=='unknown'&&p.name.toLowerCase().includes(i.family.toLowerCase())).map(p=>({plugin_id:p.plugin_id,name:p.name,version:p.version}));const verifiedKontakt=process.platform==='win32'&&hostCandidates.some(p=>p.plugin_id==='VST3:Native Instruments:Kontakt 8:f852a294'&&p.version==='8.13.0')&&['.nki','.nksn'].includes(i.format);items.push({...i,available_on_disk:exists,host_candidates:hostCandidates,bindings:c.bindings.filter(b=>b.content_id===i.id),load_requirement:verifiedKontakt?'Use kontakt_preset_import to load and verify this file, then use the returned preset ID. License and sample availability are checked only by the actual load/probe.':i.family==='Kontakt'?'Requires a verified Kontakt adapter or an explicitly identified saved state. Discovery alone does not prove the patch can play.':'Use a matching verified format adapter or saved plugin state.'});}
