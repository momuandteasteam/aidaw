...
- **書き出さずに現在の設定を聴く**：音源、トラックFX、センド／リターン、マスターFXを含む現在のプロジェクトを、WAVへレンダリングせず音声デバイスで再生できます。再生部分だけは小型のAIDAW Player画面から、曲の選択、再生、一時停止、停止、位置移動、出力先の変更を操作できます。
...
3. **AIDAW**が、プロジェクト、演奏パート、音源、ミキサー、エフェクトを管理し、直接再生または音声ファイルへの書き出しを行います。
...
AIDAWは、固定テンポ・48 kHz・ステレオの直接再生とオフライン制作に対応しています。リアルタイム録音、可変テンポ、サイドチェイン、自由なバス間ルーティングには現在対応していません。再生は開始時のプロジェクトrevisionを使うため、編集後は停止して再生し直します。AUはmacOS、VST3はmacOSとWindowsが対象です。
...
- [レンダリング不要のプレイヤー](docs/PLAYBACK.md)
