...
- **Listen without exporting a render:** Play the current project directly through the audio device, including instruments, track effects, sends, returns, and master effects, without creating a WAV file. A small AIDAW Player window is provided only for playback, with project selection, play, pause, stop, seek, and output-device controls.
